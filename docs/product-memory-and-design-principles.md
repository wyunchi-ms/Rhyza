...
- 这两个选区操作发生在阅读过程中：提交、创建节点及生成回答都应保留当前阅读路径和滚动位置，不自动切换到新节点或滚动到底部。普通发送和主动选择节点仍使用原有导航行为。
- 分支决策和 dump 文件路径只保留在后台诊断中，不在聊天界面展示；诊断失败记录日志，真实的请求错误仍需显示给用户。
- Explain 节点标题应包含引用内容的简短预览，不能只显示 `explain`；兼容已有引用数据，并保留用户自定义标题。
...
- 会话状态筛选基于叶节点的标记状态，支持多选并保留匹配叶节点的祖先路径；列表和图视图条件一致，筛选不能改变当前对话或用量总计。
- Markdown 阅读目录跟随当前聚焦的 Assistant 回答，只使用正文左侧的空白区域，不挤压正文。章节折叠保留内容挂载和预览交互状态，目录跳转与页面内查找必须能展开目标章节。
