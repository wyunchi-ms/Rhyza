...
export function prepareArchifyViewerHtml(
	...
	prepared = prepared.replace(
		/<\/head>/i,
		`<style id="rhyza-archify-viewer">${viewerStyles}</style>\n</head>`,
	);
	const hostConfig = `<script id="rhyza-archify-host">(function(){var root=document.documentElement;var theme=${JSON.stringify(options.theme)};var mode=${JSON.stringify(options.mode)};var reportQueued=false;function fitDiagram(){var svg=document.querySelector('.diagram-container svg');if(!svg||typeof svg.getBBox!=='function')return;var backgrounds=svg.querySelectorAll(':scope > rect[fill^="url(#grid)"]');var displays=[];for(var index=0;index<backgrounds.length;index+=1){displays.push(backgrounds[index].style.display);backgrounds[index].style.display='none';}try{var box=svg.getBBox();if(!box||![box.x,box.y,box.width,box.height].every(Number.isFinite)||box.width<=0||box.height<=0)return;var padding=24;var x=Math.floor(box.x-padding);var y=Math.floor(box.y-padding);var width=Math.ceil(box.width+padding*2);var height=Math.ceil(box.height+padding*2);svg.setAttribute('viewBox',[x,y,width,height].join(' '));svg.setAttribute('preserveAspectRatio','xMidYMid meet');svg.setAttribute('data-rhyza-fitted','true');svg.removeAttribute('width');svg.removeAttribute('height');}finally{for(var restoreIndex=0;restoreIndex<backgrounds.length;restoreIndex+=1)backgrounds[restoreIndex].style.display=displays[restoreIndex];}}function reportSize(){if(reportQueued)return;reportQueued=true;requestAnimationFrame(function(){reportQueued=false;fitDiagram();if(mode!=='inline')return;var body=document.body;var height=Math.ceil(Math.max(root.scrollHeight,root.offsetHeight,body?body.scrollHeight:0,body?body.offsetHeight:0));if(height>0)parent.postMessage({type:'rhyza:archify-size',mode:mode,height:height},'*');});}function enforce(){if(root.getAttribute('data-theme')!==theme)root.setAttribute('data-theme',theme);if(root.getAttribute('data-preset')!=='classic')root.setAttribute('data-preset','classic');if(root.getAttribute('data-rhyza-mode')!==mode)root.setAttribute('data-rhyza-mode',mode);root.setAttribute('data-rhyza-viewer','true');root.removeAttribute('data-present');}enforce();window.addEventListener('load',reportSize,{once:true});if(typeof MutationObserver==='function'){new MutationObserver(enforce).observe(root,{attributes:true,attributeFilter:['data-theme','data-preset','data-rhyza-mode','data-present']});}window.addEventListener('message',function(event){if(event.data&&event.data.type==='rhyza:archify-measure')reportSize();});if(document.fonts&&document.fonts.ready)document.fonts.ready.then(reportSize).catch(function(){});setTimeout(reportSize,250);setTimeout(reportSize,700);})();</script>`;
