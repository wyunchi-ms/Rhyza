...
export const archifyDiagramTypes = [
	"architecture",
	"workflow",
	"sequence",
	"dataflow",
...
export function parseArchifySource(source: string): ParsedArchifySource {
	...
	try {
		...
		throw new Error(
			`Archify source is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
	...
	if (typeof type !== "string" || !archifyTypeSet.has(type)) {
		...
		throw new Error(
			`Archify diagram_type must be one of: ${archifyDiagramTypes.join(", ")}.${received}`,
		);
	}
	const topology = extractTopology(type as ArchifyDiagramType, normalized);
	if (topology.nodes.length === 0)
		throw new Error("Archify source does not contain any supported nodes.");
...
function normalizeArchifySpec(spec: Record<string, unknown>): Record<string, unknown> {
	const diagramType =
		normalizeDiagramType(spec.diagram_type) ??
		normalizeDiagramType(spec.type) ??
...
function normalizeDiagramType(value: unknown): ArchifyDiagramType | undefined {
	...
	if (compact.includes("architecture") || compact === "component" || compact === "system")
		return "architecture";
	...
	if (compact.includes("workflow") || compact === "flowchart" || compact === "process")
		return "workflow";
	return undefined;
}
...
function convertGenericArchitecture(spec: Record<string, unknown>): Record<string, unknown> {
	...
	const referenced = new Set(
		readEdges(spec.edges).flatMap((edge) => [edge.sourceKey, edge.targetKey]),
	);
	const boundaryKeys = new Set(
		rawNodes.flatMap((node) => {
			const key = readString(node.id, node.key);
			return key &&
				String(node.kind ?? node.type).toLowerCase() === "group" &&
	...
	const componentKeys = new Set(
		genericNodes.filter((node) => !boundaryKeys.has(node.key)).map((node) => node.key),
	);
	const components = genericNodes
		.filter((node) => componentKeys.has(node.key))
		.map((node, index) => {
			const raw = rawNodes.find(
				(candidate) => readString(candidate.id, candidate.key) === node.key,
			);
	...
	const boundaries = rawNodes.flatMap((node) => {
		...
		return wraps.length > 0
			? [{ kind: "region", label: readString(node.label, node.title, node.name) ?? key, wraps }]
			: [];
	});
	return {
		schema_version: 1,
...
function normalizeMeta(
	...
	return {
		...
		locale:
			existing.locale === "en" || existing.locale === "zh-CN"
				? existing.locale
				: hasChinese
					? "zh-CN"
					: "en",
		quality_profile: "showcase",
		...(!existing.viewBox && itemCount > 0
			? {
					viewBox: [
						Math.max(900, Math.min(1400, itemCount * 150)),
						Math.max(420, Math.ceil(itemCount / 5) * 125 + 180),
					],
				}
			: {}),
	};
...
function createCanonicalIdMap(keys: string[]): Map<string, string> {
	...
	return new Map(
		keys.map((key, index) => {
			const base =
				key.replace(/[^a-zA-Z0-9_-]/g, "_").replace(/^[^a-zA-Z]+/, "") || `node_${index + 1}`;
			let id = base;
			let suffix = 2;
...
function extractTopology(
	type: ArchifyDiagramType,
	spec: Record<string, unknown>,
): { nodes: ArchifyTopologyNode[]; edges: ArchifyTopologyEdge[] } {
...
function sequenceMermaid(parsed: ParsedArchifySource): string {
	...
	for (const node of parsed.nodes)
		lines.push(`    participant ${safeId(node.key)} as ${escapeMermaidLabel(node.label)}`);
	for (const edge of parsed.edges) {
		lines.push(
			`    ${safeId(edge.sourceKey)}->>${safeId(edge.targetKey)}: ${escapeMermaidLabel(edge.label ?? "message")}`,
		);
	}
	return lines.join("\n");
}
