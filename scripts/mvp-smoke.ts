...
const tempRoot = await mkdtemp(path.join(os.tmpdir(), "rhyza-mvp-smoke-"));
...
try {
	...
	await writeFile(
		...
		"The Context Pack retrieves shared knowledge.\n",
		"utf8",
	);
	await git(repository, "init");
	await git(repository, "config", "user.name", "KnowBranch Smoke");
	await git(repository, "config", "user.email", "smoke@rhyza.local");
	...
	assert(
		indexed.every((source) => source.status === "indexed"),
		"Every source must be indexed.",
	);
	const hits = await sources.search({ query: "shared knowledge", limit: 10 });
	assert(
		hits.some((hit) => hit.path === "design.md"),
		"Full-text search did not find the docs source.",
	);
	...
	await assertRejects(
		() => sources.read(docsSource.id, "../README.md"),
		"Source traversal was not rejected.",
	);

	const stateStore = new AppStateStore(userData);
	await stateStore.save(
		repository,
		JSON.stringify({ state: { sessions: [{ id: "repo" }] }, version: 2 }),
	);
	await stateStore.save(
		docs,
		JSON.stringify({ state: { sessions: [{ id: "docs" }] }, version: 2 }),
	);
	assert(
		stateStore.load(repository)?.includes('"id":"repo"'),
		"Repository state was not isolated.",
	);
	...
	await writeFile(
		path.join(first.path, "README.md"),
		"KnowBranch changed in session one\n",
		"utf8",
	);
	const firstDiff = await worktrees.diff("session-one");
	const secondDiff = await worktrees.diff("session-two");
	assert(
		firstDiff.diff.includes("changed in session one"),
		"First session diff is missing its edit.",
	);
	assert(
		!secondDiff.diff.includes("changed in session one"),
		"Second session observed the first session edit.",
	);

	console.log(
		`MVP smoke passed: sources=${indexed.length}, hits=${hits.length}, isolated=${first.path !== second.path}`,
	);
} finally {
