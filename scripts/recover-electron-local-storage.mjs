...
const stateKey = "knowbranch-workspace-v2";
const candidates = [];

for (const file of process.argv.slice(2)) {
	const bytes = readFileSync(file);
	const entries = readTable(bytes);
	if (process.env.KNOWBRANCH_RECOVERY_DEBUG === "1") {
		process.stderr.write(`${file}: ${entries.length} entries; ${entries.slice(0, 5).map((entry) => JSON.stringify(entry.key)).join(", ")}\n`);
	}
	for (const { key, value } of entries) {
		if (!key.includes(stateKey)) continue;
		...
		try {
			...
			candidates.push({
				...
				key,
				serialized,
				score: (state.turns?.length ?? 0) * 100
			...
			if (process.env.KNOWBRANCH_RECOVERY_DEBUG === "1") {
				process.stderr.write(`Unparsed ${JSON.stringify(key)} value(${value.length})=${JSON.stringify(decodeLocalStorageValue(value).slice(0, 160))}\n`);
			}
		}
	}
}
...
if (best && process.env.KNOWBRANCH_RECOVERY_OUTPUT) {
	writeFileSync(process.env.KNOWBRANCH_RECOVERY_OUTPUT, best.serialized, "utf8");
}
process.stdout.write(JSON.stringify(best ? {
	...
	score: best.score,
	counts: best.counts,
	output: process.env.KNOWBRANCH_RECOVERY_OUTPUT ?? null,
} : null));
...
function readBlockEntries(table, offset, size) {
	...
	while (cursor < entriesEnd) {
		const shared = readVarint(block, cursor); cursor = shared.next;
		const unshared = readVarint(block, cursor); cursor = unshared.next;
		const valueLength = readVarint(block, cursor); cursor = valueLength.next;
		const suffix = block.subarray(cursor, cursor + unshared.value); cursor += unshared.value;
		const value = block.subarray(cursor, cursor + valueLength.value); cursor += valueLength.value;
	...
	while (source < input.length && target < output.length) {
		...
		if (type === 0) {
			...
			if (length < 60) length += 1;
				...
				length = 1;
				for (let index = 0; index < byteCount; index += 1) length += input[source++] * 2 ** (index * 8);
			}
