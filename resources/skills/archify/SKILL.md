...
## Rhyza GPT Harness contract

When the user prompt contains `<diagram_mode>archify</diagram_mode>`, this skill is running inside Rhyza. Read `references/rhyza-harness.md` before authoring. That host contract overrides the ordinary standalone delivery step: validate the candidate with this skill's CLI, then place the exact validated JSON in one fenced `archify` block in the final response. Rhyza performs the trusted HTML delivery itself and keeps a local Mermaid fallback. Never put generated HTML or SVG in the response.
