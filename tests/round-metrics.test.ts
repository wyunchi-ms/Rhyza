...
const request = (id: string): AgentModelRequestSnapshot => ({ id, sequence: 1, timestamp: "2026-09-13T00:00:00Z", model: "model", provider: "provider", api: "api", thinking: "off", context: { messages: [] }, usage });
const answer = (overrides: Partial<Turn> = {}): Turn => ({ id: "a", sessionId: "root", role: "assistant", content: "Answer", status: "complete", createdAt: "2026-09-13T00:00:00Z", completedAt: "2026-09-13T00:00:02Z", usage, modelRequests: [request("1"), request("2")], ...overrides });
const round = (answers: Turn[]) => ({ id: "u", sessionId: "root", parentId: null, title: "Question", answers });

test("metadata preserves every actual model and thinking mode without using current settings", () => {
	const metrics = roundMetrics(round([answer({ modelRequests: [request("1"), { ...request("2"), model: "other-model", thinking: "high" }, request("3")] })]));
...
test("execution appearance distinguishes completed, failed, active, interrupted and unsynced runs", () => {
	...
	const activity = { id: "agent" as const, label: "Agent", status: "error" as const, startedAt: "2026-09-13T00:00:00Z" };
	assert.equal(roundMetrics(round([answer({ status: "interrupted", activities: [activity] })])).status, "interrupted");
	assert.equal(roundMetrics(round([answer({ activities: [activity] })])).status, "error");
	assert.equal(roundMetrics(round([answer({ status: "complete_with_unsynced_knowledge", activities: [activity] })])).status, "error");
});
...
test("missing historical metrics stay unknown, recorded zero stays zero, and partial usage is identified", () => {
	const legacy = answer({ usage: undefined, modelRequests: undefined, completedAt: undefined, status: "interrupted" });
	...
	assert.equal(roundMetrics(round([answer({ usage: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, cost: 0 } })])).totalTokens, 0);
	assert.equal(roundMetrics(round([])).calls, 0);
});

test("fork-point metrics exclude copied usage and descendants", () => {
	const root: SessionNode = { id: "root", parentId: null, title: "Root", isRoot: true, status: "idle", progressStatus: "parked" };
	const child: SessionNode = { ...root, id: "child", parentId: "root", isRoot: false, forkedFromTurnId: "a", progressStatus: "todo" };
	const user: Turn = { ...answer(), id: "u", role: "user", usage: undefined, modelRequests: undefined };
	const original = [user, answer()];
	const copies = original.map((turn) => ({ ...turn, id: `copy-${turn.id}`, sourceTurnId: turn.id, sessionId: "child" }));
	const graph = projectConversationGraph([root, child], [...original, ...copies, { ...user, id: "child-u", sessionId: "child" }, answer({ id: "child-a", sessionId: "child" })]);
