...
test("frontend nodes map one-to-one to persistent Pi sessions with real parent paths", async () => {
	...
	try {
		...
		root.sessionManager.appendMessage({ role: "user", content: "first question", timestamp: Date.now() });
		root.sessionManager.appendMessage({ role: "assistant", content: [{ type: "text", text: "first answer" }], api: "openai-responses", provider: "test", model: "test", usage: { input: 1, output: 1, cacheRead: 0, cacheWrite: 0, totalTokens: 2, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } }, stopReason: "stop", timestamp: Date.now() });
		const persistedAfterFirstMessage = await SessionManager.listAll(sessionDir);
		assert.deepEqual(persistedAfterFirstMessage.map((session) => session.id), ["kb-root"]);
		...
		assert.equal(reopenedRoot.sessionManager.getEntries().filter((entry) => entry.type === "message").length, 2);
		...
		assert.equal(path.resolve(String(child.sessionManager.getHeader()?.parentSession)), path.resolve(root.sessionFile));
		assert.equal(child.sessionManager.getEntries().filter((entry) => entry.type === "message").length, 1);
		assert.equal(child.sessionManager.getEntries().filter((entry) => entry.type === "custom_message").length, 1);
		child.sessionManager.appendMessage({ role: "assistant", content: [{ type: "text", text: "branch answer" }], api: "openai-responses", provider: "test", model: "test", usage: { input: 1, output: 1, cacheRead: 0, cacheWrite: 0, totalTokens: 2, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } }, stopReason: "stop", timestamp: Date.now() });
		...
		nested.sessionManager.appendMessage({ role: "assistant", content: [{ type: "text", text: "nested answer" }], api: "openai-responses", provider: "test", model: "test", usage: { input: 1, output: 1, cacheRead: 0, cacheWrite: 0, totalTokens: 2, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } }, stopReason: "stop", timestamp: Date.now() });
		assert.equal(path.resolve(String(nested.sessionManager.getHeader()?.parentSession)), path.resolve(child.sessionFile));
...
test("concurrent resolution cannot create two Pi sessions for one frontend node", async () => {
	...
	try {
		...
		const [left, right] = await Promise.all([openOrCreatePiSession(options), openOrCreatePiSession(options)]);
		assert.equal(left.sessionFile, right.sessionFile);
		left.sessionManager.appendMessage({ role: "assistant", content: [{ type: "text", text: "done" }], api: "openai-responses", provider: "test", model: "test", usage: { input: 1, output: 1, cacheRead: 0, cacheWrite: 0, totalTokens: 2, cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0, total: 0 } }, stopReason: "stop", timestamp: Date.now() });
