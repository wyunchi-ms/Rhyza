...
export function useGraphFocus(
	...
	const move = useCallback(
		(maximize: boolean) => {
			...
			const zoom = maximize ? graphMaxZoom : from.zoom;
			const to = {
				x: width / 2 - (position.x + sessionGraphNodeSize.width / 2) * zoom,
				y: height / 2 - (position.y + sessionGraphNodeSize.height / 2) * zoom,
				zoom,
			};
			const started = performance.now();
			const duration =
				reduceMotion || window.matchMedia("(prefers-reduced-motion: reduce)").matches ? 0 : 360;
			const animate = () => {
	...
	useEffect(() => {
		...
		// Retry when the hidden canvas gains a size, and wait for the sidebar
		// transition to settle. Focus changes during opening must still maximize.
		const schedule = () => {
			cancel();
	...
	}, [visible, nodeId, flow, move, cancel, canvas]);
	return {
