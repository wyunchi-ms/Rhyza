	...
	return (
		<div className="flex-1 flex bg-white overflow-hidden">
			<main className="page-shell">
				...
				{tab === "knowledge" ? <KnowledgeChangeHistory /> : <div className="flex-1 min-h-0 flex flex-col"><div className="flex justify-end mb-2"><button type="button" onClick={() => void exportPatch()} className="secondary-button"><Download size={14} /> Export patch</button></div><pre className="flex-1 overflow-auto bg-gray-950 text-gray-100 p-4 rounded-lg text-xs whitespace-pre-wrap">{codeDiff ?? "Loading..."}</pre></div>}
			</main>
		</div>
	);
};
