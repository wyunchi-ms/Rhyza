	...
	return (
		<div className="page-shell">
			...
			<div className="flex gap-2 mb-6">
				...
				<button type="button" onClick={() => void search()} className="secondary-button">
					Search
				</button>
			</div>
			...
			<div className="flex-1 overflow-y-auto space-y-3">
				{sources
					...
					.map((source) => {
						...
						].filter((ref) => ref.sourceId === source.id && ref.stale).length;
						return (
							<section key={source.id} className="list-row flex items-center gap-4">
								...
								<div className="min-w-0 flex-1">
									<h2 className="font-bold text-primary truncate">{source.name}</h2>
									<p className="text-xs text-secondary truncate">{source.path}</p>
									<p className="text-xs text-gray-400 mt-1">
										{source.fileCount} text files
										{source.revision ? ` / ${source.revision.slice(0, 10)}` : ""}
										{staleRefs ? ` / ${staleRefs} stale references` : ""}
				...
				{hits.length > 0 && (
					<section className="pt-4">
						<h2 className="font-bold text-primary mb-3">Search results</h2>
						{hits.map((hit) => (
							<div
							...
							>
								...
								<p className="text-sm text-secondary mt-1 font-mono">{hit.preview}</p>
							</div>
						))}
					</section>
				)}
