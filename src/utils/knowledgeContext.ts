...
export function buildKnowledgeContext(
	...
	const rankedEntities = entities
		...
		.map((entity) => {
			...
			const searchable =
				`${entity.name} ${entity.aliases.join(" ")} ${entity.type} ${entity.summary}`.toLocaleLowerCase();
			const overlap = terms.filter((term) => searchable.includes(term)).length;
			return {
				entity,
				score: (mentioned.entityIds.has(entity.id) ? 1000 : 0) + (exact ? 100 : 0) + overlap * 10,
			};
		})
	...
	const relationLines = relations
		.filter(
			(relation) =>
				!relation.deletedAt &&
				(selectedIds.has(relation.sourceEntityId) || selectedIds.has(relation.targetEntityId)),
		)
		.slice(0, 12)
		.map(
			(relation) => `${relation.sourceEntityId} -[${relation.type}]-> ${relation.targetEntityId}`,
		);
	...
	const entityLines = rankedEntities.map(({ entity }) => {
		const content =
			mentioned.entityIds.has(entity.id) && entity.content.trim()
	...
	const diagramLines = diagrams
		.filter(
			(diagram) =>
				!diagram.deletedAt &&
				(mentioned.diagramIds.has(diagram.id) ||
					lowerPrompt.includes(diagram.name.toLocaleLowerCase()) ||
					diagram.nodes.some((node) => node.entityId && selectedIds.has(node.entityId))),
		)
		.slice(0, 5)
		.map((diagram) => {
			const structure = `Nodes: ${diagram.nodes
				.map((node) => `${node.label}${node.entityId ? ` [entity:${node.entityId}]` : ""}`)
				.slice(0, 24)
				.join(", ")}\nEdges: ${diagram.edges
				...
				.join(", ")}`;
			const diagramSource =
				mentioned.diagramIds.has(diagram.id) && diagram.mermaidSource.trim()
...
function getMentionedKnowledgeIds(prompt: string): {
