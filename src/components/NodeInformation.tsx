...
export function coreMetricValues(metrics: Metrics) {
	...
	return [
		metrics.totalTokens === undefined
			? "—"
			: `${formatTokens(metrics.totalTokens)}${metrics.partialUsage ? "+" : ""}`,
		formatCacheHitRate(metrics.cacheHitRate),
		metrics.durationMs === undefined ? "—" : formatDuration(metrics.durationMs),
		usage
			? `${usage.cost > 0 && usage.cost < 0.0001 ? "<$0.0001" : `$${usage.cost.toFixed(4)}`}${metrics.partialUsage ? "+" : ""}`
			: "—",
	];
}
...
export function NodeInformation({
	...
	useLayoutEffect(() => {
		...
		const measure = () =>
			setPosition(
				floatingPosition(anchor, element.getBoundingClientRect(), {
	...
	const cells = [
		["Total tokens", exact(metrics.totalTokens)],
		["Input tokens", exact(usage?.input)],
		["Output tokens", exact(usage?.output)],
		["Cache read", exact(usage?.cacheRead)],
		["Cache write", exact(usage?.cacheWrite)],
		["Cache hit", coreMetricValues(metrics)[1]],
		["Duration", coreMetricValues(metrics)[2]],
		["Cost", usage ? `$${usage.cost.toFixed(8)}` : "—"],
		["LLM calls", exact(metrics.calls)],
		["Model", metrics.models.join(", ") || "—"],
		["Thinking mode", metrics.thinkingModes.join(", ") || "—"],
	...
	return (
		<>
			<button
				ref={button}
				type="button"
				className="graph-runtime-compact nodrag nopan"
				aria-label={`LLM Call Details: ${coreMetricValues(metrics).join(" · ")} (Tokens · Cache hit · Duration · Cost)`}
				aria-haspopup="dialog"
				aria-expanded={Boolean(anchor)}
				aria-controls={anchor ? `llm-call-details-${nodeId}` : undefined}
				onMouseEnter={enter}
				onMouseLeave={leave}
				onFocus={open}
				onBlur={leave}
			...
			>
				{coreMetricValues(metrics).map((value, index) => (
			...
			{anchor &&
				createPortal(
					<div
						ref={panel}
						id={`llm-call-details-${nodeId}`}
						role="dialog"
						aria-label="LLM Call Details"
						className="node-information-panel nodrag nopan"
						style={position}
						onMouseEnter={hover.retain}
						onMouseLeave={leave}
						onFocus={hover.retain}
						onBlur={leave}
						onPointerDown={(event) => event.stopPropagation()}
					...
					>
						<header>
							...
							<button type="button" aria-label="Close LLM Call Details" onClick={close}>
								<X size={14} />
							</button>
						</header>
						<dl>
						...
						<CacheStatus request={cacheRequest} />
