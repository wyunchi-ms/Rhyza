...
export function GlobalSearch({ open, onClose }: { open: boolean; onClose: () => void }) {
	...
	const openResult = (result: GlobalSearchResult | undefined) => {
		...
		navigate("/");
		if (result.turn) window.dispatchEvent(new CustomEvent("rhyza:focus-turn", { detail: { turnId: result.turn.id } }));
		onClose();
	};

	return (
		<div className="global-search-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
			<section className="global-search-dialog" role="dialog" aria-modal="true" aria-label="Search all chats">
				<header className="global-search-input-row">
					...
					<input
						...
						onKeyDown={(event) => {
							...
							if (event.key === "ArrowUp") { event.preventDefault(); setActiveIndex((current) => results.length ? (current - 1 + results.length) % results.length : 0); }
							if (event.key === "Enter") { event.preventDefault(); openResult(results[activeIndex]); }
						}}
						placeholder="Search chats"
						aria-label="Search chats"
						aria-controls="global-search-results"
						aria-activedescendant={results[activeIndex] ? `global-search-result-${results[activeIndex].session.id}` : undefined}
					/>
					<kbd>Ctrl Shift F</kbd>
					<button type="button" onClick={onClose} title="Close search" aria-label="Close search"><X size={17} /></button>
				</header>
				<div id="global-search-results" className="global-search-results" role="listbox" aria-label="Chat search results">
					<div className="global-search-section-label">{query.trim() ? `${results.length} matching chats` : "Recent chats"}</div>
					{results.map((result, index) => (
						...
						>
							<span className="global-search-result-icon"><MessageSquareText size={16} /></span>
							<span className="global-search-result-copy"><strong>{result.session.title}</strong><small>{result.snippet}</small></span>
							<CornerDownLeft size={14} className="global-search-enter" />
						</button>
					))}
					{results.length === 0 && <div className="global-search-empty"><Search size={22} /><strong>No matching chats</strong><span>Try fewer words or a shorter spelling.</span></div>}
				</div>
...
function buildGlobalResults(query: string, sessions: SessionNode[], turns: Turn[]): GlobalSearchResult[] {
	...
	const results = sessions.map((session): GlobalSearchResult | null => {
		const sessionTurns = turnsBySession.get(session.id) ?? [];
		if (!normalizedQuery) {
			const latest = sessionTurns[sessionTurns.length - 1];
			return { session, turn: latest, snippet: latest ? cleanSnippet(latest.content) : "No messages yet", score: latest ? Date.parse(latest.createdAt) : 0 };
		}
		let score = fuzzyScore(normalizedQuery, session.title) * 1.4;
		let bestTurn: Turn | undefined;
		for (const turn of sessionTurns) {
			const turnScore = Math.max(fuzzyScore(normalizedQuery, turn.content), fuzzyScore(normalizedQuery, turn.summary ?? "") * 1.1);
		...
		return { session, turn: bestTurn, snippet: bestTurn ? matchingSnippet(bestTurn.content, normalizedQuery) : session.title, score };
...
function fuzzyScore(query: string, value: string): number {
	...
	if (exactIndex >= 0) return 1000 - Math.min(400, exactIndex) - Math.min(300, candidate.length / 20);
...
function cleanSnippet(value: string, limit = 150): string {
	const cleaned = value.replace(/```[\s\S]*?```/g, " code ").replace(/[#*_`>\[\]()]/g, " ").replace(/\s+/g, " ").trim();
	return cleaned.length > limit ? `${cleaned.slice(0, limit).trim()}…` : cleaned || "No text content";
}
